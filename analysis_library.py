    ...
    while True:
        ...
        if user_input.upper() == "A":
            ...
            for index in range(len(regions)):
                print(f"The average credit score of {regions[index]} is {countryScoreAvgs[index]:.2f}")

        elif user_input.upper() == "B":
            ages, balanceAvgs = balanceAgeAvg(entry_list)
            for index in range(len(ages)):
                print (f"The average account balance for ages {ages[index]} is ${balanceAvgs[index]:.2f}")

        elif user_input.upper() == "C":
            tenures, creditScoreAvgs = tenureScoreAvg(entry_list)
            for index in range(len(tenures)):
                print(f"The average credit score of tenure rating {tenures[index]} is {creditScoreAvgs[index]:.2f}")

        elif user_input.upper() == "D":
            genders, salaryGenderAvgs = salaryGenderAvg(entry_list)
            for index in range(len(genders)):
                print(f"The overall estimated salary for {genders[index]} is about ${salaryGenderAvgs[index]:.2f}")
...
def exportAnalysis(entry_list):
    ...
    analysis_obj.add_entry(key="AverageGeographyBased", keys=avg_geo_score[0], values=avg_geo_score[1])
    analysis_obj.add_entry(key="SalaryGenderBased", keys=avg_salary_gender[0], values=avg_salary_gender[1])
    analysis_obj.add_entry(key="AverageCreditScoreTenureBased", keys=avg_score_tenure[0], values=avg_score_tenure[1])
    analysis_obj.add_entry(key="AverageBalanceAgeBased", keys=avg_balance_age[0], values=avg_balance_age[1])
